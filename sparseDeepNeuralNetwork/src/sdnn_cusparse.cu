...
int main(int argc, char **argv)
{
    ...
    cusparseSafeCall(cusparseCreateMatDescr(&(descrD)));
    cusparseSafeCall(cusparseSetMatType(descrD, CUSPARSE_MATRIX_TYPE_GENERAL));
    cusparseSafeCall(cusparseSetMatIndexBase(descrD, CUSPARSE_INDEX_BASE_ZERO));
    ...
    for (size_t i = 0; i < nLayer; i++)
    {
        ...
        cusparseSafeCall(cusparseDcsrgemm2_bufferSizeExt(handle, 60000, nNeuron, nNeuron, &alpha,
        ...
        cusparseSafeCall(cusparseXcsrgemm2Nnz(handle, 60000, nNeuron, nNeuron,
                                              spmA.descr, spmA.total_non_zero, spmA.csrRowPtrA_device, spmA.csrColIndA_device,
                                              spmB.descr, spmB.total_non_zero, spmB.csrRowPtrA_device, spmB.csrColIndA_device,
                                              descrD, 0, nullptr, nullptr,
        ...
        CudaSafeCall(cudaMalloc(&(spmC.csrValA_device),
                                nnzC * sizeof(double)));
        cusparseSafeCall(cusparseDcsrgemm2(handle, spmA.num_rows, nNeuron, nNeuron, &alpha,
