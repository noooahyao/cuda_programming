...
int main(int argc, char **argv)
{
    ...
    int tmp_nnz_h;
    int *tmp_nnz_per_row, *index, *flag, *nzrows;
    CudaSafeCall(cudaMallocManaged(&(tmp_nnz_per_row), batch * sizeof(int)));
    CudaSafeCall(cudaMalloc(&(index), (batch + 1) * sizeof(int)));

    CudaSafeCall(cudaMallocManaged(&(flag), batch * sizeof(int)));
    CudaSafeCall(cudaMalloc(&(nzrows), sizeof(int)));
    int nzrowsH = 0;
    ...
    for (size_t i = 0; i < nLayer; i++)
    {
        ...
        CudaSafeCall(cudaMemset(flag, 0, batch * sizeof(int)));
        CudaSafeCall(cudaMemset(nzrows, 0, sizeof(int)));
        ...
        filter<<<1, 32>>>(tmp_nnz_per_row, flag, nzrows, batch);
        ...
        CudaSafeCall(cudaMemcpy(&nzrowsH, nzrows, sizeof(int), cudaMemcpyDeviceToHost));

        cout << "nzrowsH " << nzrowsH << endl;
        csrmcsc_compute2<<<nzrowsH * 32 / BLOCK_SIZE + 1, BLOCK_SIZE>>>(spmA.csrRowPtrA_device, spmA.csrColIndA_device, spmA.csrValA_device,
                                                                        spmB.cscColPtrA_device, spmB.cscRowIndA_device, spmB.cscValA_device,
                                                                        spmC.csrRowPtrA_device, spmC.csrColIndA_device, spmC.csrValA_device,
                                                                        index, spmA.num_rows, nNeuron, batch, bias, flag, nzrows);
